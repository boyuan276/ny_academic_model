...
%% NEW Interface Limits Constraints
    %Update Screen
        if verbose
          fprintf('  - Building Interface flow constraints.\n');
        end
    %Initial Add Constraints
        om = add_constraints(om, 'iflims', {nt, nj_max, nc_max+1});
    %Loop through each period, 
    for t = 1:nt %24 time periods
      for j = 1:mdi.idx.nj(t) 
        for k = 1:mdi.idx.nc(t,j)+1
            %Get constraint
%                 mpc = get_mpc(om);
...
%                 [B, Bf, Pbusinj, Pfinj] = makeBdc(baseMVA, bus, branch);
                ...
                ion = find(mpc.branch(:, BR_STATUS));
                [B, Bf, Pbusinj, Pfinj] = makeBdc(baseMVA, mpc.bus, mpc.branch(ion,:));
                
                n = size(Bf, 2);                    %% dim of theta (number of buses)
                ...
                %Loop through each interface
                    for kk = 1:nifs
                        %extract branch indices
                            br = ifmap(ifmap(:, 1) == ifidx(kk), 2);
                            if isempty(br)
                                error('userfcn_iflims_formulation: interface %d has no in-service branches', kk);
                            end
                        %capture signs: direction of flow on branches
                            d = sign(br);
                        %eliminate negatives in branch vector
                            br = abs(br);
                        %Initialize 
                        ...
                        %record values
                            Aif(kk, :) = Ak;
                        lif(kk) = iflims(kk, 2) / baseMVA - bk;
                        uif(kk) = iflims(kk, 3) / baseMVA - bk;
                    end
...
%                     om = add_constraints(om, 'iflims',  Aif, lif, uif, {'Va'});      %% nifs
                ...
                %uncomment this to make work again (and comment out Ray's code below)
%                   om = add_constraints(om, 'iflims', {t,j,k}, Aif, lif, uif, {'Va'});
                
                %Ray's code
                    vs = struct('name', {'Va'}, 'idx', {{t,j,k}});
                    om = add_constraints(om, 'iflims', {t,j,k}, Aif, lif, uif, vs);
